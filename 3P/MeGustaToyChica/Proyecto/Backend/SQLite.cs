...
public partial class SQLite : ContextoBD
{
	...
	public List<Interfaz> ObtenerInterfacesPorTipoUsuario(int IdTpoUsuario)
	{
	    List<Interfaz> resultado = new List<Interfaz>();
	    Excepcion = string.Empty;
	    ...
	    try
	    {
	        resultado = NvlAccInterfaces
	            .Where(n => n.IdTpoUsuarioNavigation.Id == (long)IdTpoUsuario)
	    ...
	    {
	        Excepcion = e.ToString();
	        Console.WriteLine("Error al obtener interfaces por tipo de usuario: " + e.Message);
	...
	public bool TieneAccesoAInterfaz(int IdTpoUsuario, string NombrePagina)
	{
